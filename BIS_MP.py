import numpy as np
from tkinter import Tk, Label, Button, Text, OptionMenu, StringVar, messagebox
...
class EncryptionGUI:
    def __init__(self, master):
        ...
        self.cipher_menu = OptionMenu(master, self.cipher_type, "RSA", "HILL", "Monoalphabetic", "CeaserCipher",
                                      "Polyalphabetic", "PlayfairCipher", "OneTimePad", "RailFenceCipher", )
        ...
        self.hillcipher = HillCipher("GYBNQKURP")
    ...
    def generate_key(self):
        ...
        if self.cipher_type.get() == "HILL":
            def generate_random_matrix():
                return np.array([[random.randint(0, 25) for _ in range(3)] for _ in range(3)])

            def is_key_valid(key_matrix):
                det = int(np.round(np.linalg.det(key_matrix)))
                return gcd(det, 26) == 1

            def create_key_string(key_matrix):
                return ''.join(chr(num + 65) for row in key_matrix for num in row)

            def generate_key():
                while True:
                    key_matrix = generate_random_matrix()
                    if is_key_valid(key_matrix):
                        messagebox.showinfo("Key Generated", "HILLCIPHER Key Generated Successfully")
                        return create_key_string(key_matrix)
    ...
    def encrypt(self):
        ...
        if self.cipher_type.get() == "HILL":
            print("HILL CIPHER EXECUTED")
            plaintext = self.textbox.get("1.0", "end-1c")
            encrypted_text = self.hillcipher.encrypt(plaintext, self.key)
            self.textbox.delete("1.0", "end")
            self.textbox.insert("1.0", encrypted_text)
            self.result_label.config(text="HillCipher Encryption Successful", fg="green")
        if self.cipher_type.get() == "Monoalphabetic":
            key = self.key
    ...
    def decrypt(self):
        ...
        if self.cipher_type.get() == "HILL":
            ...
            plaintext = self.hillcipher.decrypt(ciphertext, self.key)
            self.textbox.delete("1.0", "end")
...
class HillCipher:
    ...
    def create_key_matrix(self, key_string):
        key_matrix = []
        for i in range(3):
            row = [ord(char) % 65 for char in key_string[3 * i: 3 * (i + 1)]]
            key_matrix.append(row)
        return np.array(key_matrix)

    def process_text(self, text, block_size):
        text = text.upper().replace(" ", "")
        padding_size = (block_size - len(text) % block_size) % block_size
        text += 'X' * padding_size
        return text
    ...
    def matrix_to_text(self, matrix):
        return ''.join(chr(int(num) + 65) for num in matrix)

    def encrypt(self, plain_text, key):
        key_matrix = self.create_key_matrix(key)
        plain_text = self.process_text(plain_text, 3)
        plain_matrix = np.array(self.text_to_matrix(plain_text)).reshape(-1, 3)
        cipher_matrix = np.dot(plain_matrix, key_matrix) % 26
        cipher_text = self.matrix_to_text(cipher_matrix.flatten())
        return cipher_text

    def decrypt(self, cipher_text, key):
        key_matrix = self.create_key_matrix(key)
        inverse_key_matrix = self.mod_inverse_matrix(key_matrix, 26)
        cipher_matrix = np.array(self.text_to_matrix(cipher_text)).reshape(-1, 3)
        plain_matrix = np.dot(cipher_matrix, inverse_key_matrix) % 26
        plain_text = self.matrix_to_text(plain_matrix.flatten())
...
class MonoalphabeticCipher:
    ...
    def set_key(self, key=None):
        if key is None:
            ...
            if len(key) != 26 or not all(k.isalpha() and v.isalpha() for k, v in key.items()) or len(
                    set(key.values())) != 26:
